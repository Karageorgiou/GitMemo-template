# Runethread Taxonomy and Canonical Vocabulary — V1

## Purpose

This document defines the canonical vocabulary used by Runethread and gives AI/LLM operators rules for avoiding taxonomy drift.
...
A durable preference, working style, interaction rule, or constraint about how the user wants work performed or answers produced. Do not broaden scope beyond what was established.
...
A historically meaningful project/system snapshot. Use sparingly. Routine progress belongs in current-state documents or underlying project history unless the snapshot itself has durable historical value.
...
Durable procedural or explanatory knowledge worth retrieving later that does not primarily fit another memory type. Do not turn Runethread into a general encyclopedia or duplicate authoritative source repositories.
...
A newer memory replaces this memory as canonical understanding for the relevant scope. The old memory remains useful historical evidence. A superseded memory MUST have at least one valid incoming `supersedes` edge from another memory.
...
- `normal`: default durable memory.
- `high`: overlooking it could materially impair future work.
- `critical`: rare cases where failure to retrieve it could cause serious correctness, safety, security, or major project-integrity problems.

Do not use `critical` merely because a memory is interesting.

---

# 5. Provenance basis

- `user_stated`: explicitly provided by the user; authoritative for the user's own choices/preferences but not automatic verification of external claims.
...
- `derived`: follows deterministically from identified evidence.
- `inferred`: model judgment or pattern inference rather than direct evidence.
- `migrated`: imported from older stored context whose original provenance is incomplete or cannot be represented precisely.
...
Confidence is independent of provenance basis:

- `high`: strongly established for represented scope.
...
- `depends_on`: current memory materially depends on target.
...
A correction that fully replaces old canonical truth commonly contains both `corrects` and `supersedes` to the same target.

Reverse/incoming relationships are derived by tooling or indexes. Do not persist inverse fields such as `superseded_by` or `corrected_by`.
...
- `sensitive`: deserves additional caution but remains deliberately eligible for Runethread.

There is no persisted `never-store` value. Forbidden information MUST NOT be written at all. Credentials, authentication secrets, recovery codes, private keys, and similar secret material are always excluded.
...
A project is a durable body of work with enough continuity that future retrieval may need to scope by it.

Examples: `recharge`, `gaio-pepper`, `runethread`.

Do not create separate project slugs for branches, individual tickets, phases, or one-off tasks. A memory may belong to multiple projects. An empty list is valid for general/profile memories.

Operators MUST search existing project usage before creating a new slug.
...
Search phrases and natural-language synonyms generally belong in `aliases`, not duplicate tags.
...
Aliases are not canonical ontology terms; their purpose is retrieval recall. Do not add dozens of speculative aliases. Prefer phrases a future user or LLM could realistically use.

---

# 14. Entities

Entities contain `kind` plus `name`. Likely V1 kinds include:
...
Do not create a new entity kind when an existing one fits. Entity kinds remain extensible because real usage may reveal missing categories. Do not use entities merely to repeat every noun in the memory.

---

# 15. Temporal semantics

`created_at` is when the Runethread memory object was created.
...
Do not substitute memory creation time for an unknown real-world effective date. Use `null` when an effective boundary is unknown or not applicable.
...
2. look for a semantically equivalent term;
3. prefer the existing term even if another spelling feels more natural;
4. if none fits, choose the simplest stable kebab-case concept;
5. put synonyms in `aliases` rather than creating parallel canonical terms;
6. if classification is materially uncertain, preserve uncertainty rather than inventing a fake distinction.
